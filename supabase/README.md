...
Invitation emails use Supabase-generated magic links delivered through Resend. The browser exchanges the single-use token hash via `/api/auth/link`; the server derives the account from Supabase verification. Tokens stay in URL fragments and are removed from history before exchange. Existing email-code sign-in remains available for expired links. The service-role key is used only server-side to generate the recipient link and is never returned to the inviter.
