...
export interface SharingAuth { createEmailLink(email: string): Promise<string>; verifyEmailLink(tokenHash: string): Promise<AuthSession>; requestCode(email: string): Promise<void>; verifyCode(email: string, code: string): Promise<AuthSession>; refresh(refreshToken: string): Promise<AuthSession>; user(token: string): Promise<SharingAccount | null>; logout(token: string): Promise<void> }
