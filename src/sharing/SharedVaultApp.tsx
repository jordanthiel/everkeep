...
export function SharedVaultApp({ client, initialVaultId, desktop = false, onEditingChange }: { client: SharingClient; initialVaultId?: string; desktop?: boolean; onEditingChange?: (dirty: boolean, busy: boolean) => void }) {
  ...
  const accountChanged = useCallback((next: Account | null) => { generation.current++; active.current = null; setView(null); setEditing(null); setChanged(false); setVaults([]); setAccount(next) }, [])
  ...
  const open = useCallback(async (id: string) => { setBusy(true); setError(''); const gen = generation.current; try { const next = await client.get(id); if (gen !== generation.current) return; active.current = id; setView(next); setEditing(null); requestAnimationFrame(() => heading.current?.focus()) } catch (error) { setError(message(error)) } finally { setBusy(false) } }, [client])
  useEffect(() => { if (initialVaultId && vaults.some(vault => vault.id === initialVaultId && vault.status === 'active') && !active.current) void open(initialVaultId) }, [initialVaultId, vaults, open])
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      ...
      setChanged(false); setEditing(null); setConflicting(null); setNotice('Changes saved to the shared vault.'); await open(view.id)
    } catch (error) {
      if (gen !== generation.current) return
  ...
  return <main className="ek-sharing">
    ...
    {view && <>
      ...
      {editing ? <form className="ek-card" onSubmit={save}><h2>Edit {editing.title}</h2><p>Changes are attributed to {account?.email}. Structured relationships are managed in the original vault.</p>
        ...
        {view.records.length === 0 && <div className="ek-card">No records are currently available in your selection. Ask the owner to update your access.</div>}
        ...
        {view.role === 'owner' && account && <SharingControls client={client} id={view.id} snapshot={view} owner={account.email} url={url} />}
      </>}
    </>}
  </main>
}
